...
# Set exec policy to yolo (allow-all) so the cron agent can run any command
# without hitting allowlist denials. The per-binary allowlist was fragile:
# the AI agent runs commands through a shell (e.g. bash -c "curl ...") whose
# wrapper binary was never in SEED_BINS, causing repeated allowlist-miss errors.
echo "[entrypoint] setting exec policy to yolo (allow-all)"
node /app/dist/index.js exec-policy preset yolo
echo "[entrypoint] exec policy set to yolo"
